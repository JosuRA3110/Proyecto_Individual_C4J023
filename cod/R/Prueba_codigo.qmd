...
#Ahora procedemos a crear los diversos graficos y la interfaz grafica con la que interactua el usuario

#Primero cargamos lo necesario

Base.graficos <- read.csv(here("data", "Base.graficos.csv"))
Base.graficos$Año <- as.integer(Base.graficos$Año)

#Cargamos los mapas de los estados para colocrlos como poligonos
...
#Hacemos la interfaz por utilizar 
ui <- fluidPage(
...
#Aca hacemos basicamente la pagina que mantendra los graficos deseados y creara en tiempo real los graficos por estado.

server <- function(input, output, session) {
  
  estado_click <- reactiveVal(NULL)

#Hacemos el mapa de los estados

  output$map <- renderLeaflet({

    base_anio <- Base.graficos %>%
      filter(Año == input$selected_year)
      ...
      addPolygons(
        ...
        label = ~lapply(
          paste0(
            ...
            "Mortalidad: ", round(Mortalidad_cardiovascular, 1)
          ),
          htmltools::HTML
        )
      ) %>%
          ...
          map.eachLayer(function(layer) {
            if (layer.options && layer.options.layerId) {
              ...
              layer.on('dblclick', function(e) {
                Shiny.setInputValue('estado_dblclick', layer.options.layerId, {priority: 'event'});
              });
...
#Aca registramos cuando el usuario realiza el doble clic para saber si se deben desplegar los graficos dados.

  observeEvent(input$estado_dblclick, {
    estado_click(input$estado_dblclick)
    cat("Doble clic en:", input$estado_dblclick, "\n")
  })

#Aca hacemos los graficos por estado.
  ...
  output$state_plot <- renderPlot({
    ...
    ggplot(df, aes(x = Obesidad, y = Mortalidad_cardiovascular)) +
      geom_point(size = 3, alpha = 0.8) +
      geom_smooth(method = "loess", se = FALSE, linewidth = 1.3, span = 0.8) +
      labs(
